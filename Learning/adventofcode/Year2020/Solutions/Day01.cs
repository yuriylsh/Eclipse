...
namespace Solutions
{
    public class Day01
    {
        ...
        public void Part2()
        {
            ...
            var (first, second, third) = Day01Solution.ThreeSum(input, 2020);
        
            (first + second + third).Should().Be(2020);
            (first * second * third).Should().Be(120637440);
        }
    }

    class Day01Solution
    {
        public static (int first, int second) TwoSum(Span<int> input, int target)
        {
        ...
        public static (int first, int second, int third) ThreeSum(Span<int> input, int target)
        {
            for (var i = 0; i < input.Length - 2; i++)
            {
                var x = input[i];
                var complement = target - x;
                var (second, third) = TwoSum(input.Slice(i + 1), complement);
                if (second != 0 && third != 0) return (x, second, third);
            }
